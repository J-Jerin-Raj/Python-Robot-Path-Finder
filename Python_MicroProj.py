...
class Forklift:
    def __init__(self):
        self.pos = [random.randint(2, N-3), random.randint(2, N-3)]
        self.dir = random.choice([[1,0],[-1,0],[0,1],[0,-1]])
        self.size = 1  # 1x1 forklift
    ...
    def move(self, grid):
        ...
        # Check if new position is free in the grid
        if 0 <= nx < N and 0 <= ny < N:
            if grid[nx, ny] == 0:
                self.pos = [nx, ny]
...
def update(frame):
    ...
    # Move forklifts using inflated grid to avoid collisions
    for f in forklifts:
        f.move(inflated_grid)
